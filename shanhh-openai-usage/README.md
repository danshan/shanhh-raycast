...
- 以本地生成的 GitHub 风格 365 天热力图展示每日 Token 用量. 图表按 API 返回的最新 `startDate` 对齐 UTC 日历, 以周为列, 星期为行, 使用相对展示区间正值峰值的 10 级绿色强度; 零值显示为灰色, 无记录日期及其图例在深浅主题下均显示为无边框纯白色, 并显示数据截止日期和图例.
